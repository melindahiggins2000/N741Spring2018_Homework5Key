---
title: "Homework 5 2018 - ANSWER KEY"
...
# list summary of data components
# within the HMPv35 dataset
HMPv35
...
#The following commands plot the different diversity measures for the Tongue_dorsum body site.
alpha_meas = c("Observed", "Chao1", "ACE", "Shannon", "Simpson", "InvSimpson")
...
HMPv35subr2  <- transform_sample_counts(HMPv35subsub2, function(x) x / sum(x) )
...
HMPv35subfrbac2 = subset_taxa(HMPv35subfr2, Phylum=="Bacteroidetes")
HMPv35subfrbac2
...
#Calclate number of taxa in object
ntaxa(HMPv35subfrbac2)

par(mar = c(10, 4, 4, 2) + 0.1) # make more room on bottom margin
N <- 20
barplot(sort(taxa_sums(HMPv35subfrbac2), TRUE)[1:N]/nsamples(HMPv35subfrbac2), las=2)
...
title = "plot_bar; by site; Bacteroidetes only"
plot_bar(HMPv35subfrbac2, "HMPbodysubsite", "Abundance", title=title)
```
...
plot_bar(HMPv35subfrbac2, "HMPbodysubsite", "Abundance", "Phylum", title=title)
```
...
NOTE: Answers here may vary. I (Melinda) like the coloring by SEX and shapes by body type - but it depends on what you're trying to illustrate by the figure. It could easily go the other way. The colors by Body Site do show the clusters more clearly. Here are the answers for both.

In this case, since we only have 1 body site (Tongue_dorsum) - it is probably more interesting to color code by sex.

```{r}
#Place your answer here as another comment.

plot_net(HMPv35subfr2, maxdist = 0.3, color = "HMPbodysubsite", shape="sex")

plot_net(HMPv35subfr2, maxdist = 0.3, shape = "HMPbodysubsite", color="sex")
```
